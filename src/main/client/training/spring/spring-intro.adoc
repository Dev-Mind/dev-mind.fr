...
Follow this link:../outil/install-development-environment[page] to install your dev environment (IDE + Java)
...
* a company (Spring is open source Licence «Apache 2.0» but managed by a company VM Ware)
...
File : *> build.gradle* contains information used by Gradle to build app
[source,groovy]
----
plugins {
	id 'org.springframework.boot' version '2.7.2' // (1)
	id 'io.spring.dependency-management' version '1.0.12.RELEASE' // (2)
	id 'java' // (3)
}
...
** only in test (9) spring-boot-starter-test
** always in compilation mode (7) spring-boot-starter-web
...
On startup Spring Boot will automatically scan all sub packages defined below `com.emse.spring.faircorp`. If these packages contain Spring Beans (classes annotated with special annotations LIKE `@Controller`, `@Service` ...), Spring Boot will automatically add them to the Spring context.
