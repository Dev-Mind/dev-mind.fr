...
public class SensorController {
    ...
    @PostMapping // (8)
    public ResponseEntity<Sensor> create(@RequestBody Sensor sensor) { // (9)
        // We have to check that the sensor does not exist
        if (sensor.id() != null && sensorDao.existsById(sensor.id())) {
...
class SensorControllerTest {
    ...
    void shouldNotUpdateUnknownEntity() throws Exception {
        SensorEntity sensorEntity = createSensorEntity(1L, "Temperature room 1");
        Sensor expectedSensor = SensorMapper.of(sensorEntity);
    ...
    @Test
    void shouldNotCreateExistingEntity() throws Exception {
        SensorEntity sensorEntity = createSensorEntity(1L, "Temperature room 1");
        Sensor expectedSensor = SensorMapper.of(sensorEntity);
        String json = objectMapper.writeValueAsString(expectedSensor);

        Mockito.when(sensorDao.existsById(1L)).thenReturn(true);
